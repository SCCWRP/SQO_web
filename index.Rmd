...
load(file = 'data/finalsiteassess.RData')
...
sqosum <- reactive({
  ...
  out <- sqo_sum_fun(wgtavg, MCSsum, tischmthr, constants, finalsiteassess)
...
output$mcspl <- renderPlot({
  
  # input
  # nFactor <- input$nFactor
  # 
  # req(nFactor)
  ...
  p = toplo %>%
    ggplot() +
    scale_x_continuous(expand = c(0.01, 0), limits = c(0,nFactor)) + 
    scale_y_continuous(expand = c(0.01, 0), limits = c(0,1)) +
    annotate("rect", xmin = 0, xmax = nFactor, ymin = 0,    ymax = 0.25, alpha = al, fill = "red") +
    annotate("rect", xmin = 0, xmax = nFactor, ymin = 0.25, ymax = 0.5,  alpha = al, fill = "green") +
    annotate("rect", xmin = 0, xmax = nFactor, ymin = 0.5,  ymax = 0.75, alpha = al, fill = "blue") +
    annotate("rect", xmin = 0, xmax = nFactor, ymin = 0.75, ymax = 1,    alpha = al, fill = "purple") +
    stat_ecdf(aes(sitsedlnk, color = contam),
              geom = "line", size = 1.5) +
    geom_vline(xintercept = 0.5, lty = "dashed", color = "blue") +
    annotate("text", x = nFactor-1, y = .875, label = "Very Low") +
    annotate("text", x = nFactor-1, y = .615, label = "Low") +
    annotate("text", x = nFactor-1, y = .375, label = "Moderate") +
    annotate("text", x = nFactor-1, y = .125, label = "High") +
