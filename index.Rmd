...
bsaf_res <- eventReactive(input$run, {
  ...
  out <- list(res = res, contamcalc = contamcalc)
...
renderPlot({
  ...
  p1 <- ggplot(toplo1, aes(x = species, y = val)) + 
  ...
  p2 <- ggplot(toplo2, aes(x = species, y = val)) + 
    geom_bar(stat = 'identity') + 
    ylab('CBIOTA') + 
    xlab(cntbsaf) + 
    theme_bw(base_size= 16, base_family = 'serif')+ 
...
```{r}
renderDataTable({bsaf_res()$contamcalc})
```
